...
C*********************************************************************** 
  100 FORMAT(4I5) 
  104 FORMAT(4I5,f8.4,2I3,I6,I3) ! 9 objects
  101 FORMAT(13I5) 
...
C-FABIO     &DD(6,10),FF(8),HH(8),CV(9),RPQ(9),ES(20)
     &DD(6,50),FF(8),HH(8),CV(9),RPQ(9),ES(20)
      DIMENSION EE(15),DFIS(4),DFIX(4),IVME(4),IMINT(50),IMEST(50)
...
C***********************************************************************
      ...
      A(3) = TEFF
      RAG  = STBSUN*DSQRT(10.D0**ELLOG)/((10.D0**TEFF)**2)
C RAG = physical radius in solar radii
      RTOT = RAG*SUNRcm*1.D-10
C so RTOT = 6.9599*RAG
      EMSOL= EMTOT/(SUNMg33)
      A(4) = DLOG10(RAG)
      A(6) = EMSOL
      A(7) = ETA
...
C ******** STAMPA VARIE QUANTITA' SE NABLA=3 OGNI KSD MESHPOINTS *******
   ...
   58 CONTINUE
      ...
      EE( 9 )  = EPNEU
      EE( 10 ) = GRSAD
      EE( 11 ) = DRAD(JF) ! nabla_rad
      EE( 12 ) = DAD ! nabla_ad
      EE( 13 ) = CAP ! Rosseland mean opacity
      EE( 14 ) = CSP
C phi/delta = -1 (MINUS)
      EE( 15 ) = VEELCO/VSOUND
...
C      XSi28 = XXX( 22 , KM )
C      XS32 = XXX( 23 , KM )
C      Xn = XXX( 24 , KM ) ! neutrons
C      XAr36 = XXX( 25 , KM )
      XFe56 = XXX( 26 , KM )

C XXX DEFINED IN FILE 'NETWORK'
C ADDITIONAL SPECIES:
C  27  Dumy   5.0000   5.0D0   0.00000D+00
...
C N.B.: FINVMU DEFINED AT KM = K - 1
      FINVMU = (XI*2/1)+(XHE3*3/3)+(XHE4*3/4)+(XC*7/12)+(XN*8/14)+
     $ (XO*9/16)+(XD*2/2)+(XLi7*4/7)+(XBe7*5/7)+(XC13*7/13)+(XN15*8/16)
     $ +(XO17*9/17)+(XO18*9/18)+(XFe56*26/56)
...
C 1-MESH POINT WIDTH DERIVATIVE METHOD
      DR = (G(1,K) - G(1,KM))/6.9599 ! CORRECTED!!!
      DLNP = DLOG(G(3,K)*1.D+17) - DLOG(G(3,KM)*1.D+17)
      DLNT = DLOG(G(4,K)*1.D+06) - DLOG(G(4,KM)*1.D+06)
      DMASS = (G(5,K)-G(5,KM))/emtot
C 2-POINT WIDTH METHOD
      DR_2 = (G(1,KP) - G(1,KM))/6.9599 ! CORRECTED!!!
      DLNP_2 = DLOG(G(3,KP)*1.D+17) - DLOG(G(3,KM)*1.D+17)
      DLNT_2 = DLOG(G(4,KP)*1.D+06) - DLOG(G(4,KM)*1.D+06)
      DMASS_2 = (G(5,KP)-G(5,KM))/emtot

C REPEAT THE EXTRACTION OF X'S FROM 'NETWORK', BUT FOR K=KM+1, NOT KM
      XI_K = XXX( 1 , K )
      XHE3_K = XXX( 2 , K )
      XHE4_K = XXX( 3 , K )
      XC_K = XXX( 4 , K )
...
C      XSi28_K = XXX( 22 , K )
C      XS32_K = XXX( 23 , K )
C      Xn_K = XXX( 24 , K ) ! neutrons
C      XAr36_K = XXX( 25 , K )

      IF( XI_K . LE . 0. . AND . XHE4_K . LE . 0. ) THEN
      XI_K   = CARXY( 1 , K )
      XHE4_K = CARXY( 2 , K )
      ENDIF
C DEFINE FINVMU_K, THE EQUIVALENT OF FINVMU, BUT AT K = KM + 1
Cerrorbait
      FINVMU_K = (XI_K*2/1)+(XHE3_K*3/3)+(XHE4_K*3/4)
     $ +(XC_K*7/12)+(XN_K*8/14)+(XO_K*9/16)+(XD_K*2/2)
     S +(XLi7_K*4/7)+(XBe7_K*5/7)+(XC13_K*7/13)+(XN15_K*8/16)
     $ +(XO17_K*9/17)+(XO18_K*9/18)+(XFe56_K*26/56)
...
C      XSi28_P = XXX( 22 , KP )
C      XS32_P = XXX( 23 , KP )
C      Xn_P = XXX( 24 , KP ) ! neutrons
C      XAr36_P = XXX( 25 , KP )
      XFe56_P = XXX( 26 , KP )

      IF( XI_P . LE . 0. . AND . XHE4_P . LE . 0. ) THEN
      XI_P   = CARXY( 1 , KP )
      XHE4_P = CARXY( 2 , KP )
      ENDIF

C DEFINE FINVMU_P, THE EQUIVALENT OF FINVMU, BUT AT KP = K + 1
      FINVMU_P = (XI_P*2/1)+(XHE3_P*3/3)+(XHE4_P*3/4)
     $ +(XC_P*7/12)+(XN_P*8/14)+(XO_P*9/16)+(XD_P*2/2)
     S +(XLi7_P*4/7)+(XBe7_P*5/7)+(XC13_P*7/13)+(XN15_P*8/16)
     $ +(XO17_P*9/17)+(XO18_P*9/18)+(XFe56_P*26/56)

C TAKE DIFFERENCE OF THE MU'S
C 1-MESH POINT WIDTH DERIVATIVE METHOD
c      DMU = FINAL_MU_K - FINAL_MU
C 2-POINT WIDTH METHOD
c      DMU_2 = FINAL_MU_P - FINAL_MU
...
C 2-POINT METHOD
c      DXC_2 = XC_P - XC
c      DXLi7_2 = XLi7_P - XLi7
c      DXN_2 = XN_P - XN


C TAKE DIFFERENCE OF NATURAL LOG OF THE MU'S
C 1-POINT METHOD
      DLNMU = DLOG(FINAL_MU_K) - DLOG(FINAL_MU)
C 2-POINT METHOD
c      DLNMU_2 = DLOG(FINAL_MU_P) - DLOG(FINAL_MU)
...
c      DXLi7_DR = DXLi7/DR
C 2-POINT METHOD
c      DMU_DR_2 = DMU_2/DR_2 ! d(mu)/dr
C      DMU_DM_2 = DMU_2/DMASS_2 ! d(mu)/dm
c      DEL_MU_2 = DLNMU_2/DLNP_2 ! d(ln(mu))/d(ln(P))
c      DLNMU_DR_2 = DLNMU_2/DR_2 ! d(ln(mu))/dr
c      DXC_DR_2 = DXC_2/DR_2
c      DXN_DR_2 = DXN_2/DR_2
c      DXLi7_DR_2 = DXLi7_2/DR_2

C IDLE THINKING FOR THERMOHALINE MIXING IMPLEMENTATION
C LEDOUX CRITERION (CANTIELLO, LANGER, 2010):
C  0 >= DEL-DEL_AD >= (PHI/DELTA)*DEL_MU
C assume phi/delta == 1
...
C WRITE RESULTS TO STAMPE

      WRITE(2,112) x1mas,XI,XD,XHE3,XHE4,XLi7,XBe7,XC,XC13,XN,
     # XN15,XO,XO17,XO18,XFe56,KM
C WRITE GRADIENT CALCULATIONS TO SEPARATE FILE
      
C      WRITE(29,170) x1mas, FINAL_MU,DMU_DR,DMU_DR_2,DLNMU_DR,
C     # DLNMU_DR_2,DEL_MU,DEL_MU_2,DXLi7_DR,DXLi7_DR_2,DXC_DR,DXC_DR_2,
C     # DXN_DR,DXN_DR_2,KM
...
c ******* STORE MISCELLANEOUS QUANTITIES FOR GRAPHS
...
C     ENDIF
   ...
   48 CONTINUE
      ...
      RETURN
  100 FORMAT(/,' AGE:',F15.11,' DT:',F8.4,' L:',F8.4,' TE:',F8.4,' logR
     &:',F8.4,' GR:',1P,D10.2,' M:',0P,F9.5,' E:',F6.3,' DM:',1P,D10.3,
     &I2,2I5)
  ...
  108 FORMAT(///,'   M/MTOT      R/RTOT  LOG(P)  LOG(T)  LOG(RO)   L/LSU
     &P     ENUC      EGRAV     ENEU      DELTA     RADIAT    ADIAB  OPA
     &CITY   C_P    V-SOUND   MESH',/)
C 'MU  ' 
  109 FORMAT(F12.9,F10.7,F10.6,F8.5,F8.4,1P,D13.5,1P,3D11.3,1P,D9.2,1P,
     &D11.3,0P,F6.3,1P,D12.4,D12.4,D9.2,I5)
C 1x,D14.7,
...
C    &MESH ')
  111 FORMAT(1H1,/,'      Mr         H        D        HE3     HE4            
     ...
     &     O18      Fe56           MESH')
C  171 FORMAT(1H1,/,'Mr     MU      d(mu)/dr    d(mu)/dr_2   dln(mu)/dr 
C     & dln(mu)/dr_2   dln(mu)/dln(p)   dln(mu)/dln(p)_2   d(X_Li7)/dr   
C     & d(X_Li7)/dr_2  d(X_C)/dr   d(X_C)/dr_2   d(X_N)/dr   d(X_N)/dr_2
C     &    MESH')

C 112 FORMAT(F12.9,1P,6D12.5,4D10.2,I5)
  112 FORMAT(1X,F12.9,14D14.7,I5)
C  170 FORMAT(1X,F12.9,13D15.7,I5)
