...
\item Radiative zone, consisting of layers for which the Schwartzschild criterion is NOT fulfilled, thus ensuring stability against convection. ****mass For a solar mass RGB star, this extends out to ~0.29***$M_{\star}$
\item Convective zone, where the Schwartzschild criterion is fulfilled, and mixing is modelled using the mixing-length theory (MLT), with the free parameter modelled such that, given solar input parameters, the model produces solar outputs.
