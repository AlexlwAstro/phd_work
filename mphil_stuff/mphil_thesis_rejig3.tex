...
\begin{abstract}
This thesis presents tables of data derived from theoretical models of stellar atmospheres. The tables contain values of ratios of interstellar extinction in multiple filters compared to that in the well-studied Johnson-Cousins $V$ filter, denoted by $A_{X}/A_{V}$. Significant variations of the value of $A_{X}/A_{V}$ with changes in the effective temperature, surface gravity and metallicity of the stellar atmospheric models are observed in the data. Mathematical functions of these stellar parameters are proposed which accurately describe the changes in the $A_{X}/A_{V}$ data.\\*

These functions are applied to theoretical isochrones to calculate the extinction ratio in each filter to be applied to each individual star. The results are compared with the results of employing the standard method of applying a single, fixed extinction ratio value to every star. Allowing for variable extinction ratios, via the use of functions, on a given stellar population produces age estimates consistently lower than those provided by using fixed ratios. The significance of this age difference is high only in cases where a UV filter or filters from the Gaia observatory is employed. When observational Gaia data for an open cluster is studied, a fixed-ratio approach to extinction is found to significantly overestimate the cluster age, underestimate the overall interstellar reddening of the cluster and underestimate the cluster metallicity, when compared with results from a function-based approach.
\end{abstract}

%\textbf{Acknowledgements}\\*
%I would like to thank my Director of Studies, Professor Maurizio Salaris, for his expertise and guidance, and my other supervisors, Professor Nathan Bastian and Doctor Steve Longmore, for their support and advice during the project.
