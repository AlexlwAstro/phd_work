...
\item This value represents a cluster environment with an overall extinction which is high without being absurdly so, according to the $A_{V}$ range obtained by multiplying the $E_{B-V}$ values in Table 2 of ****Siegel2019 by an $R_{V}$ value of 3.1, as assumed for the synthetic data in this project. This assumption as applied to the table is justified on the basis that with a small number of exceptions, the clusters were assessed as obeying the Milky Way law for extinction as a function of wavelength, which is the same as that employed in the seminal extinction-modelling studies, such as \cite{1989ApJ...345..245C}.
\end{itemize}
