...
The incidence of absorption and scattering events depends on the wavelength of the incoming photons. This dependence is also incorporated into the optical depth via the scattering cross-section $\sigma$ in Equation \ref{optical_depth}. According to the Mie solution \citep{1908AnP...330..377M} to Maxwell's equations for scattering of EM waves by a homogeneous sphere****. If the particles in the ISM are (naively) assumed to be spherical with radius $a$, its geometric cross-section is $\pi a^{2}$ for any incoming photon. Using the scattering and geometric cross-sections, a dimensionless extinction coefficient:
...
\caption{****Plots of $A_{X}/A_{V}$ with as functions of the initial $A_{V}$ value chosen for calculation of bolometric corrections for selected ACS filters. Solid lines represent dwarfs (log(g) = 5.0).Source: \cite{2008PASP..120..583G}}
\end{center}
...
There are significant variations in the $A_{X}/A_{V}$ data generated for this project via Equations \ref{BC_extinc} and \ref{BCs_diff} as the parameters describing the model stellar atmosphere are varied. The data showing variations in $A_{X}/A_V{V}$ with effective temperature are shown for the WFC3 and Gaia filters in Figure \ref{just_data_FeH0_WFC3gaia} and for the ACS filters in Figure \ref{just_data_FeH0_ACS}. This is important as these variations are independent of any effect due to interaction of the light emitted with the ISM. \\*
