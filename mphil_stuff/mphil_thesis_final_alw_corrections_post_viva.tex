...
Observational results made using filters are often shown at the filters' effective wavelength $\lambda_{\textnormal{eff}}$, defined as an average****:

****A significant issue arises due to the fact that the value of $\lambda_{\textnormal{eff}}$ is weighted by the flux of the source itself. For different types of stars, therefore, the effective wavelength of the same filter is actually different. Therefore, when the observed stellar spectrum is calculated using the (incorrect) fixed filter $\lambda_{\textnormal{eff}}$ value, the true spectrum is obtained by multiplying the observed spectrum by a factor known as the ``colour correction''. The size of this correction is different for different filters, as they occupy different wavelength region and therefore experience different weights when calculating the true $\lambda_{\textnormal{eff}}$.
