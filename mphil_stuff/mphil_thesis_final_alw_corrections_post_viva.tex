...
The incidence of absorption and scattering events depends, in part, on the wavelength of the incoming photons. This dependence is also incorporated into the optical depth via the collision cross-section $\sigma$ in Equation \ref{optical_depth}. If a dust particle in the ISM is (naively) assumed to be spherical with radius $a$, the Mie solution \citep{1908AnP...330..377M} to Maxwell's equations can be used to estimate the extinction cross-section of the dust grains. The Mie solution describes the scattering of EM waves in the specific case in which the waves interact with homogeneous dielectric spheres,such as particles, and is accurate for all wavelengths. In the limit where $\lambda \gg a$, the Mie solution produces the standard theory of Rayleigh scattering for small particles. According to the Mie solution, a spherical dust particle has a geometric cross-section of $\pi a^{2}$. Using the scattering ($\sigma(\lambda)$) and geometric cross-sections, a dimensionless extinction efficiency factor:

\begin{equation}
Q(\lambda) = \frac{\sigma(\lambda)}{\pi a^{2}}
\label{ext_wavelength}
\end{equation}
can be defined. According**** When the photon wavelength is on the order of the dust particle size ($\lambda \sim\ a$), \cite{1908AnP...330..377M} showed that $Q \sim\ a/\lambda$. In the limit $\lambda \gg a$, Rayleigh scattering means that $\sigma(\lambda) \propto \lambda^{-4}$ and, consequently $Q$ decreases towards zero, particularly for larger particles such as dust, while for $\lambda \ll a$, $Q$ becomes constant with regard to wavelength. Typical interstellar dust grains have radii ($a$) ranging between 100 and 1000 nm \citep{2000JGR...10510299W}, but the dominant grain size is 300 nm \citep{2003JGRA..108.8030L}. Therefore, the condition $\lambda \sim\ a$ is satisfied for interstellar dust in the UV-to-near-IR spectral range, which covers approximately a 100-1000 nm wavelength range. Therefore, in this spectral range the dependence of the extinction cross-section on wavelength for relevant ISM dust particles can be calculated:
...
meaning that, for a given dust particle with known size, the extinction cross-section is proportional to $1/\lambda$, meaning that for interstellar dust particles the extinction is significantly greater in the UV than in the IR. \\*
...
Colour corrections are tools to determine the true apparent magnitudes of stars from the magnitudes observed through a particular set of filters. They are applied   to the case of systematic errors in measured fluxes arising as functions of colour. By using a standard set of magnitudes and zero points for a given type stars in the relevant filters, the colour correction terms can be determined by linking the standard values
...
\end{itemize}
The last reason itemized above is justified according to the data in Table 2 of \cite{2019AJ....158...35S}. The $A_{V}$ were obtained by multiplying the $E_{B-V}$ values by an $R_{V}$ value of 3.1, as assumed for the synthetic data in this project. This assumption as applied to the table is justified on the basis that with a small number of exceptions, the clusters were assessed as obeying the Milky Way law for extinction as a function of wavelength, which is the same as that employed in the seminal extinction-modelling studies, such as \cite{1989ApJ...345..245C}. ****Out of the 49 different open clusters in the table, only 3 have $A_{V}$ values greater than 1.0. However, the largest $A_{V}$ value calculated was 2.17 using the observational calculations made in the same paper. This result puts the $A_{V}$ value of 1.0 assumed for the data in this project near the midpoint of the total $A_{V}$ range covered by the clusters in the \cite{2019AJ....158...35S} sample.
...
\hline
\multirow{2}{*}{Filter} & \multicolumn{8}{c}{Coefficient} & \multirow{2}{*}{Max. deviation in $A_{X}/A_{V}$}\\
 & $a$ & $b$ & $c$ & $d$ & $e$ & $f$ & $A_{min}$ & $A_{max}$\\
\hline
